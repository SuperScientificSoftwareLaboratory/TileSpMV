#ifndef WARP_TILESPMV
#define WARP_TILESPMV
#include"common.h"
#include"mmio_highlevel.h"
//#include"mmio.h"
#include"utils.h"

void warplevel_csr(Beidou_Tile_Matrix *matrix, int tile_row, int tile_id, int *csr_offset, int *csrptr_offset, 
...

{
    ...
    for (int ri = 0; ri < rowlen; ri++)
    {
        ...
        y[tile_row * BLOCK_SIZE + ri] += sum;
    }

}

void warplevel_coo(Beidou_Tile_Matrix *matrix, int tile_row, int tile_id, int *coo_offset, 
                   MAT_VAL_TYPE *x, MAT_VAL_TYPE *y, int x_offset)
...
//                    MAT_VAL_TYPE *x, MAT_VAL_TYPE *y, int x_offset)
{
    ...
    for (int rj= 0; rj < tilennz; rj++)
    {
        ...
        y[tile_row * BLOCK_SIZE + coorowidx] += Tile_coo_Val[offset + rj] * x[x_offset + coocolidx]; 
    }
    
}

void warplevel_ell(Beidou_Tile_Matrix *matrix, int tile_row, int tile_id, int *ell_offset, 
                   MAT_VAL_TYPE *x, MAT_VAL_TYPE *y, int x_offset)
...
void warplevel_hyb(Beidou_Tile_Matrix *matrix, int tile_row, int tile_id, int *hyb_coocount, int *hyb_offset, 
                   MAT_VAL_TYPE *x, MAT_VAL_TYPE *y, int x_offset)
...
//                    MAT_VAL_TYPE *x, MAT_VAL_TYPE *y, int x_offset)
{
    ...
    //     y[tile_row * BLOCK_SIZE + hybrowidx] += Tile_hyb_Val[offset + blkwidth[tile_id]* rowlen + i] * x[x_offset + hyb_coo_colidx];
    // }
    
}

void warplevel_dns(Beidou_Tile_Matrix *matrix, int tile_row, int tile_id, int *dns_offset, 
                   MAT_VAL_TYPE *x, MAT_VAL_TYPE *y, int x_offset)
// (int rowA, int colA, int tilemA, int tilenA, 
//                    int *tile_pre_A, int *tile_columnidx, int *tile_nnz, int tile_row,  int tile_id, 
...
//                    MAT_VAL_TYPE *x, MAT_VAL_TYPE *y, int x_offset)
{
    ...
    //	int stop = ri == BLOCK_SIZE - 1 ? (nnzb_A[blkj+1]-nnzb_A[blkj]) : BlockA_Ptr[ri+1+blkj*BLOCK_SIZE];
        for (int rj = ri * collen; rj < (ri +1)*collen; rj++)
        {
            ...
            y[tile_row * BLOCK_SIZE +ri] += x[x_offset + densecolidx] * Tile_dns_Val[offset +densecolidx * rowlen + ri]; //Blockdense_Val[denseoffset +rj];
        }
...
void warplevel_dnsrow(Beidou_Tile_Matrix *matrix, int tile_row, int tile_id, int *dnsrow_offset, 
                      MAT_VAL_TYPE *x, MAT_VAL_TYPE *y, int x_offset)
...
//                    MAT_VAL_TYPE *x, MAT_VAL_TYPE *y, int x_offset)
{
    ...
    for (int ri=denserowptr[tile_id]; ri < denserowptr[ tile_id +1 ];ri++)
    {
        ...
        y[tile_row * BLOCK_SIZE +Tile_dnsrow_idx[ri]] += sum;
    }
    
}

void warplevel_dnscol
